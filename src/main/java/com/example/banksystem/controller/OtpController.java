...
public class OtpController implements Initializable {
    ...
    public void initialize(URL url, ResourceBundle resourceBundle) {
        ...
        Optional<BankType> bankType = bankTypeDao.get(5);       // <=====
        Names.setText("Name bank : " + bankType.get().getName());
        minsum.setText("Minimal sum : " + bankInfoList.get(4).getMinSum() );
        Maxsum.setText("Maximal sum : " + + bankInfoList.get(4).getMaxSum());
        minterm.setText("Minimal term : " + + bankInfoList.get(4).getMinTerm());
        maxterm.setText("Maximal term : " + + bankInfoList.get(4).getMaxTerm());
        percent.setText("Percent : " + bankInfoList.get(4).getPercent());
    }
    ...
    public void continueButtonOnAction(ActionEvent event) {
        try {
            ...
            Optional<BankInfo> bankInfoOpt = bankInfoList.stream().filter(t -> t.getBankId() == 5)      // <=== Міняєш АЙДІ банка
                    .findAny();
